...
router.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    try {
      ...
      const player: Player | undefined = await trx("players")
        .where({ id: player_id })
        .first();
      ...
      const team: Team | undefined = await trx("teams")
        .where({ id: team_id })
        .first();
      ...
      const updatedTeam: Team | undefined = await db("teams")
        .where({ id: team_id })
        .first();
      ...
      const teamPlayers: Player[] = await db("players")
        .where({ team_id })
        .select("*");
